...
process_execute (const char *file_name) 
{
  ...
  if (tid != TID_ERROR)
  {
	...
	if(exec.success)
		list_push_back(&thread_current()->childlist, &exec.wstatus->elem);
	else
		tid = TID_ERROR;
  }
  return tid;
...
/* A thread function that loads a user process and starts it
   running. */
static void
start_process (void *exec__)
{
  struct exec_info *exec = exec__;
  ...
  if(success)
  {
	  ...
	  exec->wstatus = curr->wstatus = malloc(sizeof *exec->wstatus);

	  lock_init(&exec->wstatus->lock);
	  sema_init(&exec->wstatus->sema,0);
	  exec->wstatus->exit_status = -1;
	  exec->wstatus->alive_cnt = 2;
	  exec->wstatus->tid = curr->tid;
  }
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  e = list_begin(&(curr->childlist) );
  while(e != list_end(&(curr->childlist) ) )
  {
	  struct wstatus *a = list_entry(e, struct wstatus, elem);
	  if(child_tid == a->tid)
	  {
		  sema_down(&a->sema);
		  list_remove(e);
...
process_exit (void)
{
  ...
  if(cur->wstatus != NULL)
  {
	  struct wstatus *a = cur->wstatus;
  	  printf("%s: exit(%d)\n", cur->name, a->exit_status);
  ...
  e = list_begin(&cur->childlist);
  while(e != list_end(&cur->childlist) )
  {
	  struct wstatus *b = list_entry(e, struct wstatus, elem);
	  next = list_remove(e);
...
static bool setup_stack_helper (const char* cmd_line, uint8_t * kpage, uint8_t * upage, void ** esp) 
{
    ...
    char *currarg, *saveptr;
	char *cmdline2;
	int argc=0;
    char **argv = malloc(2*sizeof(char *));
		
		//push entire command line
	if(!(cmdline2 = push(kpage, &ofs, cmd_line, strlen(cmd_line) + 1)  ) )
		return false; 
	if(!push(kpage, &ofs, &null, sizeof(null)) )
		return false; 	
	for(currarg = strtok_r(cmdline2, " ", &saveptr); currarg != NULL; currarg = strtok_r(NULL, " ", &saveptr) )
	{
	  argv[argc] = currarg;
	  argc++;
	}
  	...
  	for(i=argc-1; i>=0; i--)
  	{
		void *uarg = upage + (argv[i] - (char*) kpage);
